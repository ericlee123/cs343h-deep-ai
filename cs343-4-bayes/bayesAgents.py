...
def constructBayesNet(gameState):
    ...
    # get all the observation variables
    for house in gameState.getPossibleHouses():
        for obs in gameState.getHouseWalls(house):
            obsVars.append(OBS_VAR_TEMPLATE % obs)

    # get all edges that need to be created
...
def getMostLikelyFoodHousePosition(evidence, bayesNet, eliminationOrder):
    ...
    """
    "*** YOUR CODE HERE ***"
    finalFactor = inference.inferenceByVariableElimination(bayesNet, [FOOD_HOUSE_VAR], evidence, eliminationOrder)
    p = 0
    best = None
    for ass in finalFactor.getAllPossibleAssignmentDicts():
	temp = finalFactor.getProbability(ass)
	if temp > p:
	    p = temp
	    best = ass
    return best
...
class VPIAgent(BayesAgent):
    ...
    def computeEnterValues(self, evidence, eliminationOrder):
        ...
        *Do not* take into account the "time elapsed" cost of traveling to each
        ...
        "*** YOUR CODE HERE ***"
    	finalFactor = inference.inferenceByVariableElimination(self.bayesNet, HOUSE_VARS, evidence, eliminationOrder)
    	leftProb = 0
    	rightProb = 0
    	for ass in finalFactor.getAllPossibleAssignmentDicts():
    	    if ass[GHOST_HOUSE_VAR] == TOP_RIGHT_VAL and ass[FOOD_HOUSE_VAR] == TOP_LEFT_VAL:
    		    leftProb = finalFactor.getProbability(ass)
    	    if ass[GHOST_HOUSE_VAR] == TOP_LEFT_VAL and ass[FOOD_HOUSE_VAR] == TOP_RIGHT_VAL:
    		    rightProb = finalFactor.getProbability(ass)
    ...
    def computeExploreValue(self, evidence, enterEliminationOrder):
        ...
        "*** YOUR CODE HERE ***"
    	for po in self.getExplorationProbsAndOutcomes(evidence):
    	    temp = evidence.copy()
    	    temp.update(po[1])
    	    val = self.computeEnterValues(temp, enterEliminationOrder)
    	    bigger = max(val[0], val[1])
    	    expectedValue += po[0]*bigger
