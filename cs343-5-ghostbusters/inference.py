    ...
    def observe(self, observation, gameState):
        ...
        if noisyDistance is None:
            ...
            for lp in self.legalPositions:
                dist = util.manhattanDistance(lp, pacmanPosition)
                allPossible[lp] = emissionModel[dist] * self.beliefs[lp]
    ...
    def elapseTime(self, gameState):
        ...
        *** GORY DETAIL AHEAD ***
        ...
        for b in self.beliefs:
            newPostDist = self.getPositionDistribution(self.setGhostPosition(gameState, b))
            for npd in newPostDist:
                newBeliefs[npd] += newPostDist[npd] * self.beliefs[b]
...
class JointParticleFilter:
    ...
    def initializeParticles(self):
        ...
        for i in range(self.numParticles / len(perms)):
    ...
    def observeState(self, gameState):
        ...
        for p in self.particles:
            particle = p
            for j in jailed:
                particle = self.getParticleWithGhostInJail(p, j)
            prob = 1.0
            for i in range(self.numGhosts):
                if i not in jailed:
        ...
        if bellefs.totalCount() == 0:
            ...
            newParticles = []
            for i in range(self.numParticles):
                newParticles.append(util.sample(bellefs))
            self.particles = newParticles
    ...
    def elapseTime(self, gameState):
        ...
        for oldParticle in self.particles:
            ...
            "*** YOUR CODE HERE ***"
            for i in range(self.numGhosts):
                 newPostDist = getPositionDistributionForGhost(setGhostPositions(gameState, newParticle), i, self.ghostAgents[i])
                 newParticle[i] = util.sample(newPostDist)
            "*** END YOUR CODE HERE ***"
