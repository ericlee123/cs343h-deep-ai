...
class GradientDescentSolver(Solver):
    ...
    def get_updates_with_momentum(self, loss_tensor, param_vars):
        ...
        for i in range(len(param_vars)):
            ...
            updates.append((param_vars[i], param_vars[i] + vel))
    ...
    def solve(self, input_train_data, target_train_data, input_val_data, target_val_data, model, callback=None):
        ...
        "*** YOUR CODE HERE ***"
        train_dict = {model.input_ph: input_train_data, target_ph: target_train_data}
        val_dict = {model.input_ph: input_val_data, target_ph: target_val_data}
        ...
        for iter_ in range(self.iterations):
            ...
            train_loss = session.run(loss_tensor, feed_dict=train_dict)
            session.run(update_ops, feed_dict=train_dict)
...
class StochasticGradientDescentSolver(GradientDescentSolver):
    ...
    def solve(self, input_train_data, target_train_data, input_val_data, target_val_data, model, callback=None):
        ...
        for iter_ in range(self.iterations):
            ...
            train_dict = {model.input_ph: train_batch[0], target_ph: train_batch[1]}
            val_dict = {model.input_ph: val_batch[0], target_ph: val_batch[1]}
            train_loss = session.run(loss_tensor, feed_dict=train_dict)
            session.run(update_ops, feed_dict=train_dict)
            val_loss = session.run(loss_tensor, feed_dict=val_dict)
